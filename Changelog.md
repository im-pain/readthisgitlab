### Changes for V9.0.0
* Blender 5.0 support
* Major exporter improvements by **Sonogami-Rinne**!
    * Models now look very close to their in-game counterparts
        * Textures are now processed inside of Koikatsu
            * Exported textures are now very accurate since it uses the real Unity / KK shader to generate the image
            * It also means texture processing is extremely fast since it uses GPU instead of CPU
    * Face shapekeys now look very close to their in-game counterparts
        * This is because the armature is now exported with all bones set to a scale of 1
        * In blender, they are scaled back to their original values, and the shape keys' min max values are setup
        * It also means blink shape keys will no longer clip into the face
    * The pushup checkbox was removed. It's always enabled now
* Major importer improvements by **Sonogami-Rinne**!
    * Major improvements to import times
    * New simplified material setup
        * You can return to the legacy KKBP 8.0 material setup by selecting a material in the material list and clicking "Edit"
    * More headmods will import
* Rigify is now the default armature choice
    * Switch to the FK armature to use the atlas and .fbx export features
    * If you always want to use the FK armature, you can set your defaults in Edit > Preferences > Add-ons > KKBP menu 
* You can bulk export / bulk cards now
    * Enable the Bulk Export checkbox and choose your card folder to bulk export
    * Open the KKBP Extras section of the panel and click the bulk import button to bulk import
    * This is an experimental feature that seems to work around 80% of the time
    * If there's an error during bulk import, it will show up as "Error - CharacterName.blend".
    * You can try re-exporting the character manually if a bulk export does not work properly
* Upgrades to the Atlas feature by **AnalogKnight**!
    * You can now exclude the pupils from the atlas for easier material setup in other programs
    * You can now put more textures into an atlas, like an alpha mask atlas or a detail mask atlas for a more customizable material setup in other programs
* Fixed gag eyes clipping into the face for small chibi characters
* Accessory bones for different outfits get their own bone collections
* Eyebrow shape keys were moved to the bottom of the shapekey list
* Solid view is colorful now
* Removed "Import Studio item" button
* Other Importer UI simplifications
* Added readme files for the folders that contain the import and export script files
